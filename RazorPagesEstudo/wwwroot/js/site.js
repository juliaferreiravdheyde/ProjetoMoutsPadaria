...
$(document).ready(function () {
        ...
        if (existingProduct.length > 0) {
            ...
            $('#produtoList').append(`
                <div class="form-check d-flex align-items-center mb-3" data-product-id="${selectedProductId}">
                    ...
                    <input type="number" name="quantidades" class="form-control quantity" value="1" min="1" data-old-quantity="1" />
    ...
    $('#produtoList').on('change', '.quantity', function () {
        ...
        totalQuantity = $('#produtoList').find('.quantity').toArray().reduce((total, input) => total + (parseInt($(input).val()) || 0), 0); // Ensure valid parsing
        $('#totalQuantity').text(totalQuantity);
        $('#totalPrice').text(totalPrice.toFixed(2));
    });
